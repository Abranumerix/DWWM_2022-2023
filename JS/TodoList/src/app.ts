...
const handleSubmit = (e: Event) => {
    ...
    const newTask: TaskInterface = {
        date: new Date(),
        task: inputTask.value, // .valeur permet de récupérer ce que l'utilisateur renseigne dans le champ
...
// Bouton TOUT EFFACER
const clearTasks = () => {
    const confirmDel: boolean = confirm("Êtes-vous sûr de vouloir tout effacer ?")
    if (confirmDel === true){
    tasks.length = 0;
    saveTasks();
